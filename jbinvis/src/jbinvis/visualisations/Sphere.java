...
//    private static int normalization(int x, int y, int z, float mean, float variance) {
//    	
//    	return x,y,z;
//    }
...
//        	int sq_c = (c-127)^2;
//        	
//        	// converting from cube coordinates to sphere coordinates
//        	x = (int) ((a-127)*Math.sqrt(1- (sq_b)/2 - (sq_c)/2 + (sq_b*sq_c/3))+127);
//        	y = (int) ((b-127)*Math.sqrt(1- (sq_c)/2 - (sq_a)/2 + (sq_a*sq_c/3))+127);
//        	z = (int) ((c-127)*Math.sqrt(1- (sq_a)/2 - (sq_b)/2 + (sq_a*sq_b/3))+127);
//        	
...
//        	buffOff=4*(z*sideCountSq + y*sideCount + x);
        	...
        	// from binary files to polar coordinates
        	r = (sideCount*c) / (127*2);
        	theta = 2*((b*Math.PI/127) - (Math.PI/2));
        	phi = (2*Math.PI) - (a*Math.PI/127);
